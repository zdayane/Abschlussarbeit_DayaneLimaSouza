...
import { getNews, getUser, getUserIDFromJWT, rejectNews, useBoardIDContext, useUserIDContext } from '../backend/boardapi';
    ...
    const handleConfirmation = async () => {
        try {
            if (newsID && currentNews) {
                const resp = await rejectNews(newsID, currentNews.title, currentNews.text, currentNews.category, currentNews.journalistId, currentNews.status)
    ...
    return (
        <>
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                ...
                <Modal.Body>Are you sure you want to send  the feedback to author?</Modal.Body>
                <Modal.Footer>
            ...
            <>
                <Button variant="primary" onClick={handleAction}>
                    <h6>
                        <ChatLeftDots /> Send feedback to the author
                    </h6>
                </Button>
            </>
