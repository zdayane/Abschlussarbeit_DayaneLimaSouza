...
import {  editNewsDraft, getNews,  getUser, getUserIDFromJWT, useBoardIDContext, useUserIDContext } from '../backend/boardapi';
import { useNavigate, useParams } from 'react-router-dom';
import { NewsResource, NewsStatus, UserResource, } from '../BoardResources';
import {Pen} from 'react-bootstrap-icons';


function EditJornalistButton() {
    ...
    const handleConfirmation = async () => {
        try {
            if (newsID && currentNews) {
                const resp = await editNewsDraft(newsID, currentNews.title, currentNews.text, currentNews.category, currentNews.journalistId, NewsStatus.DRAFT);
...
export default EditJornalistButton;
